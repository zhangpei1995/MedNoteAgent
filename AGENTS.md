...
- 合并、变基或解决冲突后，必须重新核对 README、`docs/README.md`、AGENTS 和相关指南是否仍描述当前真实代码；不得把另一分支的旧接口、旧文档路径或已删除能力带回来。
...
本项目曾在合并时保留了旧分支 README 内容，导致根 README 引用了不存在的 `docs/architecture/*` 文档，并描述了当前代码中已经不存在的 demo-agent、工具注册、会话审计和 SSE 接口。后续处理合并、变基、冲突或大段文档改动时，必须按以下顺序复查：

1. 用 `git status --short` 确认工作区状态，必要时查看合并提交或冲突解决范围。
2. 用 `rg -n "<<<<<<<|=======|>>>>>>>" .` 确认没有冲突标记残留。
3. 用 `rg --files` 反查 README、`docs/README.md`、推荐阅读顺序、目录树和文档索引中的每个文件路径。
4. 用 CodeGraph 或 `rg` 核对 README 中列出的接口、类、注解、配置项和运行能力是否真实存在；不存在的说明要删除，计划项要明确标注为后续建设。
5. 删除或重写已过时的说明，不为了保留“历史脉络”留下失真的旧接口、旧路径或旧架构描述。
6. 运行匹配范围的测试或检查，并在最后执行 `codegraph sync`。

## 架构与模块边界

- 保持 `controller`、`agent`、`agent.tool`、`agent.runtime`、`service.spi`、`service.impl`、`domain`、`dto`、`config` 的职责分离。
- 当前尚未存在的职责包（例如 `agent.tool`、`agent.runtime`）只能作为明确的新能力边界引入；文档描述必须标注其当前状态，不能把计划中的包写成已实现能力。
