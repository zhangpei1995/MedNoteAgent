...
- 本项目没有 demo 概念。所谓“先跑通”是指按正式工程方式开发核心流程的最小可用闭环，只限制本阶段范围和验收深度，不使用临时 demo、试验入口或与正式链路割裂的实现边界。
...
- 核心流程跑通也必须进入正式模块边界和正式调用链；可以小步交付、保留后续增强空间，但不得新增与正式能力并行的 demo 类、demo 接口、demo 配置或 demo 文档概念。

## 代码表达要求

- 编写类、方法和参数时要清楚表达“做什么、什么时候用、怎么用”。
- 命名、JavaDoc 或必要注释应让后续开发者能理解职责、入参含义、返回结果和使用约束。
- 新增 Controller、Service 接口、DTO、领域实体或扩展点时，必须补充必要 JavaDoc：说明使用时机、关键参数含义、可空规则、状态流转和返回结果语义。
- 注释应服务于契约理解和医学安全追溯，不重复代码字面含义；不得用空泛注释掩盖命名不清或边界不清。
- 新增 Java 数据承载类、DTO、实体类等样板代码时，统一使用 Lombok 管理 getter、setter、toString、equals/hashCode 等重复方法；不得手写原生 getter/setter 堆叠样板代码。
