...
public class TemplateAnswerGenerator implements AnswerGenerator {
    ...
    public TemplateAnswerGenerator(QianwenClient qianwenClient) {
    ...
    public String generate(AnswerGenerationContext context) {
        ...
        try {
            ...
            // Local answer generation should still be runnable without network or a valid API key.
        }
        return generateTemplateAnswer(context.question(), context.riskLevel(), context.evidence());
    }

    public String buildUserPrompt(AnswerGenerationContext context) {
        return """
                任务主题：%s
                用户问题：%s
                风险等级：%s
                ...
                请生成可追溯的中文医学说明书问答。
                """.formatted(context.topic(), context.question(), context.riskLevel(), formatEvidence(context.evidence()));
    }

    private String generateTemplateAnswer(String question, String riskLevel, List<EvidenceChunk> evidence) {
        if (evidence.isEmpty()) {
            return "未在 fixture 知识库中检索到足够证据，暂不能给出基于说明书的回答。请补充药品名称或具体章节。";
        }
