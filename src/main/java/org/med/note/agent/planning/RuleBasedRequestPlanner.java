...
/**
 * Rule-based request understanding service used by the planning tool.
 */
@Service
public class RuleBasedRequestPlanner implements RequestPlanner {
    ...
    @Override
    public RequestPlanner.Plan plan(String question) {
        ...
        String topic = deriveTopic(instructions);
        List<String> taskKeywords = taskKeywords(text, queryTargets, riskSignals, instructions);
        String rewrittenQuery = rewriteQuery(topic, question, intent, taskKeywords);
    ...
    private String deriveTopic(List<String> recommendedInstructions) {
        if (recommendedInstructions == null || recommendedInstructions.isEmpty()) {
            return "药品说明书问答";
        }
        if (recommendedInstructions.size() == 1) {
            return recommendedInstructions.get(0) + "问答";
        }
        return "药品说明书问答";
    }
